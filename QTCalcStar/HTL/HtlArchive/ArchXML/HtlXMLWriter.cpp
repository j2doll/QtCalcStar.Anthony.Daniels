...
namespace HTL
{
	...
	HtlXMLWriter::HtlXMLWriter(void)
	{
	...
	void HtlXMLWriter::Set_ptrElement(HtlElement * ptrElement)
	{
		if(ptrElement)
		{
	...
	HtlElement * HtlXMLWriter::Get_ptrElement(void)
	{
		return m_ptrElement;
	};

	//!Sets the number of indent spaces per level
	void HtlXMLWriter::Set_intNumIndentSpaces(unsigned int intNumIndentSpaces)
	{
	...
	unsigned int HtlXMLWriter::Get_intNumIndentSpaces(void)
	{
		return m_intNumIndentSpaces;
	};

	//!Save the XML Element based with or without formatting
	int HtlXMLWriter::SaveXMLElement(HtlElement * ptrElement, std::string & strOutput, bool blnWithFormatting)
	{	
		/*
		//SAVE ELEMENT
		START THE OPENTAG
	...
	void HtlXMLWriter::WriteIndent(std::string & strOutput, unsigned int intLevel)
	{
		unsigned int i, j;
	...
	int HtlXMLWriter::WriteOpenTag(HtlElement * ptrElement, std::string & strOutput, bool blnWithFormatting)
	{
	...
	int HtlXMLWriter::WriteValue(HtlElement * ptrElement, std::string & strOutput, bool blnWithFormatting)
	{
	...
	int HtlXMLWriter::WriteComment(HtlComment * ptrComment, std::string & strOutput, bool blnWithFormatting)
	{
		//write a carriage return if necessary for formatting
		std::string strValue;
		if(blnWithFormatting){strOutput += '\n';};
	...
	int HtlXMLWriter::WriteProcess(HtlProcess * ptrProcess, std::string & strOutput, bool blnWithFormatting)
	{
		//write a carriage return if necessary for formatting
		int i,intNumAttributes;
	...
	int HtlXMLWriter::WriteCloseTag(HtlElement * ptrElement, std::string & strOutput, bool blnWithFormatting)
	{
		//write a carriage return if necessary for formatting
		if(blnWithFormatting){strOutput += '\n';};
		//write the open tag indent
	...
	bool HtlXMLWriter::IsAllWhiteSpace(const std::string & strInput)
	{
...
//!Update the indexes
int HtlXMLWriter::UpdateIndexes(HtlElement * ptrCurrTop)
{
...
int HtlXMLWriter::SaveXMLTree(HtlElement * ptrCurrTop,std::string & strOutput,bool blnWithFormatting)
{
	int intReturn = 0;
...
int HtlXMLWriter::SaveXMLTree(HtlElement * ptrCurrTop, std::ostream & strStreamOut,bool blnWithFormatting)
{
	int intReturn = 0;
	//hand it off to the XMLWriter
...
int HtlXMLWriter::SaveXMLTree(HtlElement * ptrCurrTop, std::ofstream & strStreamOut,bool blnWithFormatting)
{
	int intReturn = 0;
	//hand it off to the XMLWriter
	std::string strFileOutPut;
