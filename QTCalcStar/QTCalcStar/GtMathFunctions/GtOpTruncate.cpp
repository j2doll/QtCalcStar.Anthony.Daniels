...
namespace GT
{
	namespace GtCalc
	{
		...
		//!Default Constructor
		GtOpTruncate::GtOpTruncate() : GtOpBase()
		{
			m_intNumInputs = 2;
		};
		//!Default Destructor
		GtOpTruncate::~GtOpTruncate() {};

		//!Perform the operation on the stack
		int GtOpTruncate::OpEval(std::vector<GtRpnUnit> & arrObjCalcStack, bool & blnCalcSuccessful, int intCurrPos)
		{
			...
			try {
				...
				if (blnValid)
				{
					...
					unsigned int intNumDecimal = int(Math::hround(dblNumDecimal, 0));
					dblOutput = Math::htrunc(dblNumber, intNumDecimal);
