...
namespace GT
{
	namespace GtCalc
	{
		...
		int GtOpFloor::OpEval(std::vector<GtRpnUnit> & arrObjCalcStack, bool & blnCalcSuccessful, int intCurrPos)
		{
			...
			try {
				...
				if (blnValid)
				{
					...
					dblOutput = Math::hfloor(dblInput1);
