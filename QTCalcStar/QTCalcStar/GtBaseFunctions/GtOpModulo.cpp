...
namespace GT
{
	namespace GtCalc
	{
		...
		//!Default Constructor
		GtOpModulo::GtOpModulo() : GtOpBase()
		{
			m_intNumInputs = 2;
		};
		//!Default Destructor
		GtOpModulo::~GtOpModulo() {};

		//!Perform the operation on the stack
		int GtOpModulo::OpEval(std::vector<GtRpnUnit> & arrObjCalcStack, bool & blnCalcSuccessful, int intCurrPos)
		{
			...
			try {
				...
				if (blnValid)
				{
					...
					double dblNum,dblDiv;
					this->GetOpNumber(&(arrObjCalcStack.at(intCurrPos - 1)), dblDiv);
					this->GetOpNumber(&(arrObjCalcStack.at(intCurrPos - 2)), dblNum);

					dblOutput = (int)dblNum % (int)dblDiv;
