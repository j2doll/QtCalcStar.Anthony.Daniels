...
namespace GT
{
	namespace GtCalc
	{
		...
		//!Default Constructor
		GtOpVariance::GtOpVariance() : GtOpBase()
		{
			m_intNumInputs = 3;
		};
		//!Default Destructor
		GtOpVariance::~GtOpVariance() {};

		int GtOpVariance::GatherCells(std::vector<GtRpnUnit> & arrObjCalcStack, int intCurrPos)
		{
		...
		int GtOpVariance::ValidOpInputs(std::vector<GtRpnUnit> & arrObjCalcStack, bool & blnValidInputs, int intCurrPos, int intNumInputs)
		{
			int i;
			blnValidInputs = true;
		...
		int GtOpVariance::OpEval(std::vector<GtRpnUnit> & arrObjCalcStack, bool & blnCalcSuccessful, int intCurrPos)
		{
			...
			try {
				...
				if (blnValid)
				{
					...
					int i, iNum;
					double dblNum, dblSum, dblMean, dblSquareDif, dblSquareSum, dblStVar;
					...
					if (iNum > 0)
					{
						...
						dblStVar = (dblSquareSum / ((float)(iNum - 1)));
					...
					objOutput.m_dblToken = dblStVar;
					sprintf_s(chrOutput, "%f", dblStVar);
