...
#include ".\GtOpStandardDeviation.h"
...
namespace GT
{
	namespace GtCalc
	{
		...
		//!Default Constructor
		GtOpStandardDeviation::GtOpStandardDeviation() : GtOpBase()
		{
			m_intNumInputs = 4;
		};
		//!Default Destructor
		GtOpStandardDeviation::~GtOpStandardDeviation() {};

		int GtOpStandardDeviation::GatherCells(std::vector<GtRpnUnit> & arrObjCalcStack, int intCurrPos)
		{
		...
		int GtOpStandardDeviation::ValidOpInputs(std::vector<GtRpnUnit> & arrObjCalcStack, bool & blnValidInputs, int intCurrPos, int intNumInputs)
		{
			int i;
			blnValidInputs = true;
		...
		int GtOpStandardDeviation::OpEval(std::vector<GtRpnUnit> & arrObjCalcStack, bool & blnCalcSuccessful, int intCurrPos)
		{
			...
			try {
				...
				if (blnValid)
				{
					...
					int i, iNum;
					double dblNum, dblSum, dblMean, dblSquareDif, dblSquareSum, dblStDev;
					dblSum = 0.0f; dblSquareSum = 0.0f;
					...
					if (iNum > 0)
					{
						...
						dblMean = dblSum / ((float)iNum);

						for (i = 0; i < iNum;i++)
						{
							dblNum = (m_arrCells.at(i)->m_strData.toDouble());
							dblSquareDif = pow((dblNum - dblMean),2);
							dblSquareSum += dblSquareDif;
						}
						dblStDev = pow((dblSquareSum / ((float)(iNum - 1))), 0.5);
					...
					objOutput.m_dblToken = dblStDev;
					sprintf_s(chrOutput, "%f", dblStDev);
