...
namespace GT
{
	namespace GtCalc
	{
		...
		//!Default Constructor
		GtOpDegrees::GtOpDegrees() : GtOpBase()
		{
			m_intNumInputs = 1;
		};
		//!Default Destructor
		GtOpDegrees::~GtOpDegrees() {};

		//!Perform the operation on the stack
		int GtOpDegrees::OpEval(std::vector<GtRpnUnit> & arrObjCalcStack, bool & blnCalcSuccessful, int intCurrPos)
		{
			...
			try {
				...
				if (blnValid)
				{
					...
					double dblRadians;
					this->GetOpNumber(&(arrObjCalcStack.at(intCurrPos - 1)), dblRadians);
					dblOutput = dblRadians * (180.0f / pi);
