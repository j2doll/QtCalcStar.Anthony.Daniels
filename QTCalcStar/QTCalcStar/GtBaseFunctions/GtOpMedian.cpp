...
namespace GT
{
	namespace GtCalc
	{
		...
		//!Default Constructor
		GtOpMedian::GtOpMedian() : GtOpBase()
		{
			m_intNumInputs = 2;
		};
		//!Default Destructor
		GtOpMedian::~GtOpMedian() {};

		int GtOpMedian::GatherCells(std::vector<GtRpnUnit> & arrObjCalcStack, int intCurrPos)
		{
		...
		int GtOpMedian::ValidOpInputs(std::vector<GtRpnUnit> & arrObjCalcStack, bool & blnValidInputs, int intCurrPos, int intNumInputs)
		{
			int i;
			blnValidInputs = true;
		...
		int GtOpMedian::OpEval(std::vector<GtRpnUnit> & arrObjCalcStack, bool & blnCalcSuccessful, int intCurrPos)
		{
			...
			try {
				...
				if (blnValid)
				{
					...
					if (iNum > 0)
					{
						for (i = 0; i < iNum;i++)
						{
							arrVals.push_back( (m_arrCells.at(i)->m_strData.toDouble()));
						}
					}else {
					...
					std::sort(arrVals.begin(), arrVals.end());
					int iMedian = round((float)iNum / 2.0);
					double dblMedian = arrVals.at(iMedian);


					objOutput.m_objTokenType = NUMBER;
					objOutput.m_dblToken = dblMedian;
					sprintf_s(chrOutput, "%f", dblMedian);
