...
namespace GT
{
	namespace GtCalc
	{
		...
		//!Default Constructor
		GtOpMinimum::GtOpMinimum() : GtOpBase()
		{
			m_intNumInputs = 2;
		};
		//!Default Destructor
		GtOpMinimum::~GtOpMinimum() {};

		int GtOpMinimum::GatherCells(std::vector<GtRpnUnit> & arrObjCalcStack, int intCurrPos)
		{
		...
		int GtOpMinimum::ValidOpInputs(std::vector<GtRpnUnit> & arrObjCalcStack, bool & blnValidInputs, int intCurrPos, int intNumInputs)
		{
			int i;
			blnValidInputs = true;
		...
		int GtOpMinimum::OpEval(std::vector<GtRpnUnit> & arrObjCalcStack, bool & blnCalcSuccessful, int intCurrPos)
		{
			...
			try {
				...
				if (blnValid)
				{
					...
					int i, iNum;
					double dblNum, dblMin;
					dblOutput = 0.0f;
					iNum = m_arrCells.size();
					if (iNum > 0)
					{
						dblMin = (m_arrCells.at(0)->m_strData.toDouble());
						for (i = 0; i < iNum;i++)
						{
							dblNum = (m_arrCells.at(i)->m_strData.toDouble());
							if (dblNum < dblMin) { dblMin = dblNum; };
						}
					}else {
					...
					objOutput.m_dblToken = dblMin;
					sprintf_s(chrOutput, "%f", dblMin);
