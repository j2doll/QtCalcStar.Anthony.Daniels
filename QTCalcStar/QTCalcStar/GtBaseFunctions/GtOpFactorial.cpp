...
namespace GT
{
	namespace GtCalc
	{
		...
		//!Default Constructor
		GtOpFactorial::GtOpFactorial() : GtOpBase()
		{
			m_intNumInputs = 1;
		};
		//!Default Destructor
		GtOpFactorial::~GtOpFactorial() {};




		//!Perform the operation on the stack
		int GtOpFactorial::OpEval(std::vector<GtRpnUnit> & arrObjCalcStack, bool & blnCalcSuccessful, int intCurrPos)
		{
			...
			try {
				...
				if (blnValid)
				{
					...
					intNum = gRound(dblNum2);
					dblOutput = gFactorial(intNum);
