...
namespace GT
{
	namespace GtCalc
	{
		...
		int GtOpMean::GatherCells(std::vector<GtRpnUnit> & arrObjCalcStack, int intCurrPos)
		{
		...
		int GtOpMean::ValidOpInputs(std::vector<GtRpnUnit> & arrObjCalcStack, bool & blnValidInputs, int intCurrPos, int intNumInputs)
		{
			int i;
			blnValidInputs = true;
		...
		int GtOpMean::OpEval(std::vector<GtRpnUnit> & arrObjCalcStack, bool & blnCalcSuccessful, int intCurrPos)
		{
			...
			try {
				...
				if (blnValid)
				{
					...
					objOutput.m_dblToken = dblMean;
					sprintf_s(chrOutput, "%f", dblMean);
