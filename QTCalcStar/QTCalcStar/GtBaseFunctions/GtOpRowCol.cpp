...
namespace GT
{
	namespace GtCalc
	{
		...
		//!Default Constructor
		GtOpRowCol::GtOpRowCol() : GtOpBase()
		{
			m_intNumInputs = 2;
		};
		//!Default Destructor
		GtOpRowCol::~GtOpRowCol() {};

		//!Perform the operation on the stack
		int GtOpRowCol::OpEval(std::vector<GtRpnUnit> & arrObjCalcStack, bool & blnCalcSuccessful, int intCurrPos)
		{
			...
			try {
				...
				if (blnValid)
				{
					...
					this->GetOpNumber(&(arrObjCalcStack.at(intCurrPos - 1)), jCol);
					this->GetOpNumber(&(arrObjCalcStack.at(intCurrPos - 2)), iRow);
					dblOutput = 0;
					if (m_ptrSheet)
					{
						QTSpreadSheet * ptrSheetCast = dynamic_cast<QTSpreadSheet *>(m_ptrSheet);
						if(ptrSheetCast)
						{
							GtCellData * ptrCell = ptrSheetCast->GetCellPtr((long)iRow, (long)jCol);
							if (ptrCell)
							{
								dblOutput = (ptrCell->m_strData.toDouble());
							}//good cell check
					...
					objOutput.m_ptrSheet = dynamic_cast<QTSpreadSheet *>(m_ptrSheet);
